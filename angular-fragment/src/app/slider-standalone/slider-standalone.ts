import { afterEveryRender, Component, ElementRef, inject, input, PLATFORM_ID, viewChild } from '@angular/core';
...
@Component({
  selector: 'app-slider-standalone',
  standalone: true,
  imports: [EventImage],
  templateUrl: './slider-standalone.html',
  styleUrl: './slider-standalone.scss',
})
export class SliderStandalone {
  ...
  initSwipeButtons(options: SwipeControllerOptions = {}) {
    const { stepPx, startIndex = 0, loop = false } = options;
